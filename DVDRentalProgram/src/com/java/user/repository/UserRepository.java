...
public class UserRepository {
	...
	public void addUser(User user) {
		...
		String sql = "INSERT INTO users "
				...
				+ "VALUES (users_seq.NEXTVAL,?,?)"; //공백 주의!
		...
		try(Connection conn = connection.getConnection();
			...
			} //사실 이 시점은 프런트뷰를 지난 상태기 때문에 실패할 리가 없어야만 한다.
			// 때문에 나중에는 이 검사를 따로 적용하지 않음.
	...
	public List<User> findByUserName(String userName) {
		...
		try(Connection conn = connection.getConnection();
			...
			while(rs.next()) {
				Grade grade = Grade.valueOf(rs.getString("grade")); //받아서 grade타입으로 변환
				User user = new User(
				...
				userList.add(user); //포장된 객체를 리스트에 추가
			}
